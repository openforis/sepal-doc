...
- :btn:`<fa-solid fa-cloud-download-alt>`: Retrieve data.
...
Parameters
""""""""""
Select :btn:`<fa-solid fa-cloud-download-alt>` to open the **Download parameters** window. You will be able to select the measure to use on each observation of the time-series. This measure can be selected in the list of available bands presented above in a previous section.
...
When all the data is selected, select the :btn:`<fa-solid fa-check> apply` button. Notice that the task tab in the lower-left corner of the screen (1) will change from :btn:`<fa-solid fa-tasks>` to :btn:`<fa-solid fa-spinner>`, meaning that the tasks are loading.
